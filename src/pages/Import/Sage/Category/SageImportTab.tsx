...
type Product     = { prodEId: number; name: string; prc: string; thumbPic: string };
type ProductDetail = Product & { description?: string; sku?: string; brand?: string; [key: string]: any };
type Step        = 1 | 2 | 3 | 4;
    ...
    {STEPS.map((s, i) => {
      ...
      return (
        <React.Fragment key={num}>
          ...
          {i < STEPS.length - 1 && (
            <span style={{ color: "#d1d5db", margin: "0 4px", fontSize: 16 }}>→</span>
          )}
        </React.Fragment>
      );
    })}
...
const SageImportTab: React.FC = () => {
  ...
  // ── Go back ──
  const handleBack = () => {
    if (step === 2) { setStep(1); setSelectedSupplier(null); }
    else if (step === 3) { setStep(2); setSelectedProduct(null); setProductDetail(null); }
    else if (step === 4) { setStep(3); setImportStatus("idle"); }
  };
  ...
  const fetchPage = async (page: number) => {
    ...
    setProductsLoading(true);
    try {
      const res = await SearchSageProductsAPI(selectedCategory.title, selectedSupplier.supplierId, page, pagination.limit);
      const data = res?.data?.data;
      setProducts(data?.products || []);
    ...
    <div className="sage-import-tab">
        ...
        <div className="sage-import-overlay">
          <div className="sage-import-overlay__card">
            ...
            <p className="sage-import-overlay__msg">
              Importing <strong>{pagination.total || "all"}</strong> products from{" "}
              <strong>{selectedSupplier?.supplierName}</strong>. This may take a few
              minutes for large catalogues — please don't close this tab.
            </p>
      ...
      <div className="sage-body">
        ...
        <div className="sage-panel">
          ...
          {/* Supplier list – appears after category chosen */}
          {step >= 2 && (
            <>
        ...
        <div className="sage-preview">
          ...
          {step === 2 && (
            <div className="sage-preview__placeholder">
              <div className="sage-preview__placeholder-icon">🏭</div>
              <span>Now choose a supplier to load their products</span>
            </div>
          )}
          ...
          {step === 3 && !productsLoading && !selectedProduct && (
            <>
              <div className="sage-panel__label">Step 3 · Review Products</div>
              ...
              {detailLoading ? (
                ...
                <div className="sage-preview__detail">
                  ...
                  <div className="sage-preview__detail-meta">
                    <div className="sage-preview__detail-name">{productDetail?.name}</div>
                    <div className="sage-preview__detail-price">{productDetail?.prc}</div>
                    {productDetail?.sku && (
          ...
          {/* Step 4 – post-import (overlay handles the in-progress state) */}
          {step === 4 && importStatus === "done" && (
            <div className="sage-preview__placeholder">
              ...
              <span>
                Products from <strong>{selectedSupplier?.supplierName}</strong> have
      ...
      <div className="sage-footer">
        <div className="sage-footer__info">
          {selectedCategory && <>Category: <strong>{selectedCategory.title}</strong></>}
          {selectedSupplier && (
