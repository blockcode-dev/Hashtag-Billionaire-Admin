...
function ComingSoon({ name }: { name: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 20px", gap: 16, color: "#94a3b8" }}>
      <div style={{ fontSize: 56 }}>🚧</div>
      <h3 style={{ fontSize: 22, fontWeight: 700, color: "#475569", margin: 0, fontFamily: "'DM Sans', sans-serif" }}>
        {name} — Coming Soon
      </h3>
      <p style={{ fontSize: 14, margin: 0, textAlign: "center", maxWidth: 340 }}>
        We're working on integrating the <strong>{name}</strong> supplier. Check back soon!
      </p>
    </div>
  );
}
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, opacity: active || done ? 1 : 0.4 }}>
      <div
        style={{
          ...
          fontSize: 13, fontWeight: 700, flexShrink: 0, transition: "background 0.3s",
        }}
      >
        {done ? "✓" : number}
      </div>
      <span style={{ fontSize: 13, fontWeight: active ? 600 : 400, color: active ? "#1e3a5f" : "#64748b" }}>
        {label}
      </span>
    </div>
  );
...
function SearchableList({
  ...
  return (
    <div>
      ...
      {loading ? (
        <div style={{ padding: 20, textAlign: "center", color: "#94a3b8", fontSize: 13, border: "1.5px solid #e2e8f0", borderRadius: 8 }}>
          ⏳ Loading…
        </div>
      ) : items.length === 0 ? (
        <div style={{ padding: 20, textAlign: "center", color: "#94a3b8", fontSize: 13, border: "1.5px dashed #e2e8f0", borderRadius: 8 }}>
          {emptyText}
        </div>
      ) : (
        <div style={{ maxHeight: 280, overflowY: "auto", border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#fff" }}>
          {items.map((item, i) => {
            ...
            return (
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 600, color: isSelected ? "#1d4ed8" : "#1e293b" }}>
                  {getLabel(item)}
                </div>
                {getSubLabel && (
                  <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 1 }}>{getSubLabel(item)}</div>
                )}
              </div>
            );
          })}
...
function StickyActionBar({
  ...
  product: any; selectedStyle: string; selectedBrand: string; importing: boolean;
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <span style={{ fontSize: 12, color: "#64748b" }}>{selectedBrand} · Style {selectedStyle}</span>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {(["‹", "›"] as const).map((arrow, idx) => {
          const disabled = idx === 0 ? currentIndex <= 0 : currentIndex >= totalStyles - 1;
          return (
            <button
              key={arrow}
      ...
      >
        {importing ? (
          <>
            <span style={{ display: "inline-block", width: 14, height: 14, border: "2px solid #fff", borderTop: "2px solid transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
            Importing…
          </>
        ) : <>🚀 Import Product</>}
      </button>
  ...
  return (
    <div
      style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "space-between",
        gap: 16, flexWrap: "wrap", marginBottom: 20, animation: "slideIn 0.3s ease",
      }}
    >
        ...
        {hasNext && (
          <button
            ...
            style={{
              ...
              gap: 6, boxShadow: "0 2px 10px rgba(22,163,74,0.4)", whiteSpace: "nowrap",
            }}
          >
            Next Product →
          </button>
...
function SSActivewearTab() {
  ...
  const loadStyleByIndex = async (index: number, stylesList: Style[] = styles) => {
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 24 }}>
        ...
        <div>
          ...
          {loadingProduct && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 320, gap: 14, color: "#64748b" }}>
          ...
          {product && !loadingProduct && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)", borderRadius: 12, padding: "20px 24px", color: "#fff" }}>
                ...
                <h3 style={{ margin: "0 0 4px", fontSize: 20, fontWeight: 700 }}>{product.product?.name}</h3>
                <div style={{ fontSize: 13, opacity: 0.8 }}>
                  Style ID: <strong>{selectedStyle}</strong> &nbsp;·&nbsp; Brand: <strong>{selectedBrand}</strong>
                </div>
                <div style={{ display: "flex", gap: 24, marginTop: 16, flexWrap: "wrap" }}>
                  ...
                  <div>
                    <div style={{ fontSize: 22, fontWeight: 800 }}>
                      ${Math.min(...product.variants.map((v: Variant) => v.pricing?.piece ?? 0)).toFixed(2)}
                    </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: "'DM Sans', sans-serif" }}>
                    <thead>
                      <tr style={{ background: "#f8fafc" }}>
                        {["SKU", "Variant", "Stock", "Piece $", "Sale $", "Retail $"].map((h) => (
                        ...
                        <tr key={v.sku} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                          ...
                          <td style={{ padding: "10px 16px", borderBottom: "1px solid #f1f5f9" }}>
                            <span style={{
                              ...
                              color: v.stock > 10 ? "#16a34a" : v.stock > 0 ? "#b45309" : "#dc2626",
                          ...
                          {["piece", "sale", "retail"].map((key) => (
                            <td key={key} style={{ padding: "10px 16px", color: "#475569", borderBottom: "1px solid #f1f5f9" }}>
                              ${(v.pricing?.[key as keyof typeof v.pricing] ?? 0).toFixed(2)}
                            </td>
                          ))}
                        </tr>
                      ))}
...
const SSImportPage = () => {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", background: "#f1f5f9", padding: "32px 24px" }}>
      <style>{`
      ...
      <div style={{ marginBottom: 28, maxWidth: 1100, marginInline: "auto" }}>
        ...
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#0f172a" }}>Product Import Center</h1>
        <p style={{ margin: "6px 0 0", color: "#64748b", fontSize: 14 }}>
          Browse supplier catalogues, preview products, and import them into your store.
        </p>
      </div>

      <div style={{ maxWidth: 1100, marginInline: "auto", background: "#fff", borderRadius: 16, boxShadow: "0 4px 24px rgba(0,0,0,0.07)", overflow: "hidden" }}>
        {/* Tabs */}
        <div style={{ display: "flex", borderBottom: "1.5px solid #e2e8f0", background: "#f8fafc", overflowX: "auto" }}>
          {TABS.map((tab) => {
        ...
        <div style={{ padding: 28 }}>
          ...
          {activeTab === "autocat" && <ComingSoon name="Ottocap" />}
        </div>
      </div>
    </div>
  );
